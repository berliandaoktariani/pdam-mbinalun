<?php  
	...
	if (isset($_POST['id_pelanggan'])) {
		...
		@$tarif_permtr = $tarif['tarif_permtr'];
	...
	// Pastikan variabel $meter_akhir dan $mawal adalah numerik sebelum melakukan operasi penambahan
	if(is_numeric($meter_akhir) && is_numeric($mawal)) {
		$jumlah_meter = $meter_akhir + $mawal;
	} else {
		// Tangani jika salah satu atau kedua variabel bukan numerik
		$jumlah_meter = 0; // Atau berikan nilai default yang sesuai
	}

	// Pastikan variabel $meter_akhir dan $tarif_permtr adalah numerik sebelum melakukan operasi matematika
	if(is_numeric($meter_akhir) && is_numeric($tarif_permtr)) {
		$jumlah_bayar = ($meter_akhir * $tarif_permtr) + 5000;
	} else {
		$jumlah_bayar = 0; 
	}
	...
	if (isset($_POST['bsimpan'])) {
		// Lakukan penyimpanan data tanpa validasi Meter Akhir dan Meter Awal
							...
							<form method="post">
								<div class="col-md-12">
									...
									<div class="form-group">
										<?php  
										  if (@$_GET['id']=="") {?>
											<input type="submit" name="bsimpan" class="btn btn-primary btn-lg btn-block" value="SIMPAN" style="background: #7990ff;">
										  <?php }else{ ?>
											<input type="submit" name="bubah" class="btn btn-success btn-lg btn-block" value="UBAH" style="background: #7990ff;">
										<?php } ?>

										<a href="?menu=penggunaan" class="btn btn-danger btn-lg btn-block" style="background: #f08080;">RESET</a>
									</div>
								</div>
							</form>
						</div>
