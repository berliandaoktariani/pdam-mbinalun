			...
			<?php if(isset($_GET['excel'])){ ?>
				...
				<?php }else{ ?>
					<td style="margin-top: -20px;" width="15%" valign="top">
						<img src="../images/logo_pdam2.png" width="90px" height="90px">
					</td>
				<?php } ?>
			<td>
				<h4 style="margin-top: 10px;margin-left: -50px;">BADAN USAHA MILIK DESA</h4>
				<h1 style="margin-top: -20px;margin-left: -50px;" >MBINALUN</h1>
				<h5 style="margin-top: -20px;margin-left: -50px;">Kab. PakPak Bharat</h5>
			</td>
		</tr>
					...
					<?php  
						...
						if ($data=="") {
							...
							<tr>
								...
								<td align="right"><?php $aksi->rupiah($r['tarif_permtr']) ?></td>
							</tr>
					...
					<?php  
						...
						if ($data=="") {
							...
							<?php  
								if($cek >= 3){
									$no++; 
									$sum = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id_pelanggan,COUNT(bulan) as bln_tunggak,sum(jumlah_bayar) jml_bayar,SUM(jumlah_meter) as jml_meter,tarif_permtr FROM tagihan WHERE id_pelanggan = '$r[id_pelanggan]' AND status = 'Belum Bayar'"));
									...
									<tr>
										...
										<td align="right"><?php $aksi->rupiah($sum['tarif_permtr']); ?></td>
		...
		<?php }elseif (isset($_GET['riwayat_penggunaan'])) {?>
			<table width="100%" border="1" cellspacing="0" cellpadding="3">
				<thead>
					...
					<th><center>Jumlah Meter</center></th>
					<th><center>Tarif/Mtr</center></th>
					<th><center>Jumlah Bayar + Beban (Rp.5000)</center></th>
				</thead>
				<tbody>
					<?php  
						$no = 0;
