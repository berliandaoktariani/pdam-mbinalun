<?php  
	...
	if (isset($_POST['bcari'])) {
		$text = $_POST['tcari'];
		$cari = "WHERE id_tarif LIKE '%$text%' OR kode_tarif LIKE '%$text%' OR daya LIKE '%$text%' OR golongan LIKE '%$text%' OR tarif_permtr LIKE '%$text%'";
	}else{
		$cari="";
	}
						...
						<div class="panel-body">
							<form method="post">
								<div class="col-md-12">
									...
									<div class="form-group">
										<label>TARIF/MTR</label>
										<input type="text" name="tarif" class="form-control" placeholder="Masukan Tarif" required value="<?php echo @$edit['tarif_permtr']; ?>" onkeypress='return event.charCode >=48 && event.charCode <=57'>
									</div>
									<div class="form-group">
										<label>BEBAN</label>
										<input type="text" name="beban" class="form-control" placeholder="Masukan Beban" required value="<?php echo @$edit['tarif_beban']; ?>" onkeypress='return event.charCode >=48 && event.charCode <=57'>
									</div>
									...
									<div class="form-group">
										...
										<a href="?menu=tarif" class="btn btn-danger btn-lg btn-block" style="background: #f08080;">RESET</a>
									</div>
											...
											<?php  
												...
												if ($data=="") {
													...
													<tr>
														...
														<td><?php $aksi->rupiah($r['tarif_permtr']) ?></td>
														<td><?php $aksi->rupiah($r['beban']) ?></td>
														<?php  
															if($cek > 0){?>
																<td>&nbsp;</td>
															<?php }else{ ?>
